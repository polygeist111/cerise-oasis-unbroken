...
namespace LobbyRelaySample
{
    ...
    {   
        ...
        public Action<LGameState> onGameStateChanged;
        public LocalLobbyList LobbyList { get; private set; } = new LocalLobbyList();

        public LGameState LocalGameState { get; private set; }
        public LobbyManager LobbyManager { get; private set; }
        [SerializeField]
        LobbySetupInGame m_setupInGame;
        [SerializeField]
        LobbyCountdown m_countdown;
        ...
        public async void CreateLobby(string name, bool isPrivate, string password = null, int maxPlayers = 4)
        {
            ...
            {
                SetGameState(LGameState.JoinMenu);
        ...
        public void UIChangeMenuState(LGameState state)
        {
            var isQuittingGame = LocalGameState == LGameState.Lobby &&
                m_LocalLobby.LocalLobbyState.Value == LobbyState.InGame;

            if (isQuittingGame)
            {
                //If we were in-game, make sure we stop by the lobby first
                state = LGameState.Lobby;
                ClientQuitGame();
            }
            SetGameState(state);
        }
        ...
        void SetGameState(LGameState state)
        {
            var isLeavingLobby = (state == LGameState.Menu || state == LGameState.JoinMenu) &&
                LocalGameState == LGameState.Lobby;
...
#pragma warning restore 4014
        ...
        void SetLobbyView()
        {
            Debug.Log($"Setting Lobby user state {LGameState.Lobby}");
            SetGameState(LGameState.Lobby);
